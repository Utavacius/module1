# name = input("Введите Ваше имя: ")
# current_year = 2024
# date_of_birth = int(input("В каком году Вы родились? "))
# age = current_year - date_of_birth
# print("Здарова,", name)
# print("В этом году Вам ", age, "лет" )
#print("Привет, я лысая кегля в Мастере 0 LP".replace('Привет' , 'Здарова'))
# my_string = input('Stroka: ')
# print(len(my_string))
# print(my_string.upper())
# print(my_string.lower())
# print(my_string.replace(' ', ''))
# print(my_string[0])
# print(my_string[-1])
# (f'Age: {age}')
immutable_var = 9, False, 'Cat', [5, 18]
print(f'Immutable tuple: {immutable_var}')
mutable_list = ['Dog', 'Parrot', 'Kitty']
mutable_list[0] = 'Mouse'
print(f'Mutable list: {mutable_list}')
